...
if (!React || !ReactDOM) {
  ...
  if (typeof React.version == 'string') {
    ...
    if (parseInt(bVer[0]) == 0 && parseInt(bVer[1]) < 14) console.log('warning: react@>=0.14 is required, current is ' + React.version);
  }
...
if (typeof reactRequire_ == 'function' && reactModules_) {
  ...
  var _module = { exports: {}, id: 9998, loaded: true };
  __webpack_require__.c[9998] = _module;
...
},{"./react_widget":2,"./template":3}],2:[function(require,module,exports){
(function (process){
'use strict';
...
W.$templates = {};
W.$css = [];
W.$main = { $$onLoad: [], $onLoad: [], isReady: false, inRunning: false, inDesign: false, isStart: false };
W.$idSetter = {};
W.$creator = {};
W.$staticNodes = [];

var _textableTag = { // all inline tag (with <span :t=XX>) is 'true' 
  Div: true, GroundDiv: true, Article: true, Section: true,
  Header: true, Footer: true, Aside: true, Nav: true, Main: true, P: true,
  Details: true, Li: true, Blockquote: true, Pre: true,
  H1: true, H2: true, H3: true, H4: true, H5: true, H6: true,
  Caption: true, Figcaption: true, Address: true, Dd: true, Dt: true,
  Thead: true, TFoot: true, Th: true, Td: true,
  OptDiv: true, OptLi: true
};
...
W.$creator.textableBlock = function (sTemplate) {
  // for block tag, all inline tag is textable
  return !!_textableTag[sTemplate];
};
    ...
    xmlHttp.onreadystatechange = function () {
      if (xmlHttp.readyState == 4) {
        ...
        if (status == 200 && resText) {
          if (dataType !== 'json') {
            if (req.success) req.success(resText, statusText, xmlHttp); // take as plain text
          } else {
            // take as json
            var jsonData,
                isErr = true;
...
utils.loadingEntry = function (require, module, exports) {
  ...
  if (window.W !== W) {
    ...
    if (W.__debug__ || W.__design__) window.W = W; // regist for debugging
  }
    ...
    function scanJsxNode(container) {
      ...
      function scanNodeAttr(node, sPrefix, idx) {
        ...
        for (var i2 = 0; i2 < attrLen; i2++) {
          var item = node.attributes[i2],
              sName = item.name;
          if (sName == ':t' || sName == '$') {
            if (node.nodeName == 'DIV') sTemplate = item.value;else if (node.nodeName == 'SPAN') {
              isSpan = true;
              sTemplate = item.value;
            }
            if (sName == '$') {
              bProp.push([sName, sTemplate]);
        ...
        if (sTemplate) {
          ...
          bProp.forEach(function (item, idx) {
            var sAttr = item[0].toLowerCase(),
                sValue = item[1];
            var b = sAttr.split('-');
          ...
          if (node.children.length == 0 && (isSpan || _textableTag[sTemplate])) {
            // <span :t=Span>htmlText</span>
            var sText = node.textContent; // sText has decoded, such as: &lt;&gt;  --> <>
      ...
      function jsxJsonParse(sJson, sAttr, sPrefix, idx) {
        try {
          ...
          console.log('invalid JSON format: ' + sPrefix + '[' + idx + '].' + sAttr);
        }
        return undefined;
      }
...
},{"_process":4,"react":undefined,"react-dom":undefined}],3:[function(require,module,exports){
'use strict';
...
function getElementKey_(child) {
  var s = child.key || '';
  if (s[0] == '.') {
    if (s[1] == '$') return s.slice(2);else return s.slice(1);
  } else return s;
}
...
utils.setChildren = function (comp, children, callback) {
  ...
  if (!gui.inSync) {
    if (gui.compState >= 2) comp.setState({ id__: identicalId() }, callback); // trigger re-render
    else {
        // maybe duals.id__ == 1
        setTimeout(function () {
...
utils.getWTC = function (cls) {
  ...
  if (tp == 'string') {
    ...
    for (var i = 0; i < iLen; i += 1) {
      var item = cls[i];
      if (item && typeof item == 'string') Object.assign(ret, utils.getWTC(item));
    }
  }
  // else, ignore
...
ex.regist('evalInfo', function () {
  ...
  if (callspace) {
    ...
    if (Array.isArray(bTrigger)) ret = bTrigger[0].$info;
  }
      ...
      Object.keys(ret).forEach(function (sKey) {
        ...
        if (root2) {
          ...
          if (root2) {
            ...
            if (item.hasOwnProperty('value')) item.time = { $set: tm };
            // else, ignore
          }
        }
      });
...
ex.regist('setChecked', function (sPath, newOpt) {
  ...
  if (comp) {
    ...
    if (targ && targ.props['isOption.']) targ.setChecked(null, false, newOpt); // callback=null, force=false
  }
});
...
ex.regist('setVar', function (sName, value) {
  ...
  if (wdgt) {
    ...
    if (callspace) space = callspace.exprSpace;else {
      var b = comp.$gui.triggerInfo; // for trigger='...'
      if (Array.isArray(b)) space = b[0];
    }
  }
...
ex.regist('jsonp', function (req, initValue) {
  ...
  req.callback = function (ret) {
    if (comp && sKey && comp.duals.hasOwnProperty(sKey)) comp.duals[sKey] = ret; // ret maybe null, if req.notifyError is true and request failed
  };
  utils.jsonp(req);
...
ex.regist('ajax', function (req, initValue) {
  ...
  req.success = function (ret) {
    if (comp && sKey && comp.duals.hasOwnProperty(sKey)) comp.duals[sKey] = { status: 'success', data: ret };
  };
  req.error = function (xhr, statusText) {
    if (comp && sKey && comp.duals.hasOwnProperty(sKey)) comp.duals[sKey] = { status: statusText || 'error', data: null };
  };
  utils.ajax(req);
...
function evalInSpace(bAst, rootSpace) {
  ...
  if (tp == 31) return parseFloat(b[0][1]);else if (tp == 32) {
    ...
    if (sId == 'null') return null;else if (sId == 'undefined') return undefined;else if (sId == 'true') return true;else if (sId == 'false') return false;else if (sId == 'NaN') return NaN;else {
      ...
      if (ret === undefined && !rootSpace.hasOwnProperty(sId)) throw new Error('symbol (' + sId + ') not defined');else return ret;
    }
  } else if (tp == 33) {
    var s = b[0][1];
...
function findComponent_(comp, pathFlag, bInfo, parentIdx) {
  ...
  if (typeof pathFlag == 'number') {
    ...
    while (wdgt) {
      ...
      if (wdgt.$callspace && wdgt.$callspace.hasOwnProperty('flowFlag')) {
        // has callspace
  ...
  if (tp >= 51) {
    ...
    if (tp == 32) {
      ...
      var exprL = b[0],
          ...
          opName = op[1];
      if (opName == '.') {
        if (dualFlag = isCallableId(exprL)) {
          // ID --> ID(), such as: props duals count item index
          if (exprR[0] == 32) {
          ...
          if (adjustExprAst(exprL, bDepend)) {
        ...
        if (dualFlag) {
          ...
          if (exprR !== null) adjustExprAst(exprR, bDepend);
        }
      } else if (opName == '[') {
        if (dualFlag = isCallableId(exprL)) {
          // ID[sAttr]
    ...
    return [function (comp, flowDirFlag) {
      ...
      try {
        if (ctrlFlag == 0) {
          ...
          if (isExprSetter) comp.duals[sKey] = evalInSpace(bAst, space);else comp.state[sKey] = evalInSpace(bAst, space);
    ...
    if (sCtrlFlow || gui.forExpr) {
      ...
      if (gui.forExpr && (ownerWdgt = comp.widget)) {
        ...
        ctrlValue.forEach(function (item, idx) {
          for (var i = 0, child; child = comps_[i]; i += 1) {
            ...
            if (!fn) {
              ...
              newCompIdx[keyid] = newComps.push(React.createElement(childInline ? 'span' : 'div', dProp)) - 1;
            } else {
        ...
        allOldKey.forEach(function (sKey) {
          if (!newCompIdx.hasOwnProperty(sKey)) gui.removeNum += 1; // will fire duals.childNumId
        });
      }
    }
...
utils.popWin = {
  showWindow: function showWindow(ele, popOption, callback, optComp) {
    ...
    if (optComp && optComp.props['isOption.'] && optComp.state.hasOwnProperty('data-checked')) oldChecked = optComp.state['data-checked'];

    // ele should come from template or compObj.fullClone()
    popObj.setChild(React.createElement(MaskPanel__, { popOption: popOption, popFrame: popFrame }, ele), function () {
      if (W.__design__) {
...
function getCompRenewProp_(oneObj) {
  ...
  oneObj.$gui.dataset.forEach(function (item) {
    var value = oneObj.state[item];
    if (value !== undefined) props[item] = value; // take all data-xx aria-xx as stated props
  });
  bStated.forEach(function (item) {
    var value = oneObj.state[item];
    if (value !== undefined) props[item] = value;
  });
...
function deepCloneReactEle_(srcEle, dProp, compWdgt, compObj) {
  ...
  bComp.forEach(function (child) {
    ...
    if (hasClass_(child.props.className, 'rewgt-static')) {
      ...
      bArgs.push(React.createElement(childInline ? 'span' : 'div', props));
      return;
    }
    ...
    if (subObj.props['isTemplate.'] && !W.__design__) {
      ...
      if (tempObj instanceof templateNode) return React.cloneElement(child, { template: tempObj }, null);else return; // invalid template
    }
  ...
  if (bArgs.length == 0) return React.cloneElement(srcEle, dProp, null);else {
    bArgs.unshift(dProp);
    bArgs.unshift(srcEle);
    return React.cloneElement.apply(null, bArgs);
  }
}
  ...
  if (compObj) {
    ...
    if (sLink !== undefined) {
      ...
      // else, system error, unknown format
        ...
        compObj.$gui.dataset.forEach(function (item) {
          // take data-* aria-* as stated props
          var value = objState[item];
          if (value !== undefined) dProp[item] = value;
        });
        bStated.forEach(function (item) {
          var value = objState[item];
          if (value !== undefined) dProp[item] = value;
        ...
        if (!compObj.props['noSaveChild.'] && sLink === undefined) {
          ...
          bComp.forEach(function (child) {
            ...
            if (child_) dumpReactTree_(bSubRet, child_, sPath);else {
              if (hasClass_(child.props.className, 'rewgt-static')) {
                if (!compNode) compNode = ReactDOM.findDOMNode(compObj);
...
function staticDbClick(event) {
  ...
  if (!sPath) return;
  var tillNode = ReactDOM.findDOMNode(this);
  if (!tillNode) return;
  ...
  popDesigner_(staticNode, 'default', {
    name: 'default',
    title: 'edit content',
    url: '/app/files/rewgt/web/edit_static.html', halfScreen: true,
    // icon: '', noMove: false,
    // left: 0.05, top: 0.05,
    width: 0.96,
    height: 0.9,
    ...
    set: function set(node, outValue, beClose) {
      ...
      var compNode = ReactDOM.findDOMNode(compObj),
          staticNode = null;
...
function loadReactTreeEx_(bRet, bTree, bStatic, dTempSet, sPrefix) {
  ...
  function createSysEle(bRet, bChild, iFrom) {
    ...
    for (var i = iFrom; i < iLen; i++) {
      ...
      if (tp == 'string') {
        ...
        bRet.push(React.createElement(item));
        continue;
      }
      ...
      if (Array.isArray(item) && item.length >= 1) {
        ...
        } else if (React.isValidElement(sName)) isEle = true;else if (tp2 != 'function') {
        ...
        if (iChildNum) {
          ...
          if (isEle) bRet.push(React.cloneElement(sName, dProp, bRet_));else bRet.push(React.createElement(sName, dProp, bRet_));
        } else {
          if (isEle) bRet.push(isNoneProp ? sName : React.cloneElement(sName, dProp));else bRet.push(React.createElement(sName, dProp));
        }
      }
    }
  }
  ...
  if (!sName) {
    ...
    if (bHtml.length > 0) {
      ...
      bRet.push(React.createElement('div', { className: 'rewgt-static', name: idx + '' }));
  ...
  if (lnkTemp) {
    bRet.push(React.createElement(getTemplate(lnkTemp, sPrefix_), dProp));
    return false;
  }
  ...
  if (iTagType > 0) {
    ...
    if (iChildNum > 0) {
      ...
      if (iTagType == 3) bRet.push(React.cloneElement(objTemp, dProp, bRet));else bRet.push(React.createElement(objTemp, dProp, bRet_));
    } else {
      if (iTagType == 3) bRet.push(isNoneProp ? objTemp : React.cloneElement(objTemp, dProp));else bRet.push(React.createElement(objTemp, dProp));
    }
  } else {
    ...
    if (iChildNum > 0) {
      ...
      for (var i = 1, item; item = bTree[i]; i += 1) {
        ...
        } else if (React.isValidElement(item)) bArgs.push(item);else if ((tp = typeof item === 'undefined' ? 'undefined' : _typeof(item)) == 'string') bArgs.push(React.createElement(Span__, { 'html.': item }));else if (tp == 'function') bArgs.push(React.createElement(item));
        // else, ignore
      }
      if (hasStatic) bArgs[1] = Object.assign({}, dProp, { 'hasStatic.': true });
    }
    bRet.push(React.createElement.apply(null, bArgs));
  }
...
function streamReactTree_(bTree, iLevel) {
  ...
  if (sName == 'RefDiv') {
    ...
    if (iFlag == 3) sRet += '<span :t=' + sName;else sRet += '<div :t=' + sName;
  }
  ...
  for (var sKey in dProp) {
    ...
    if (typeof value == 'string' && (!value || value[0] != '{' || value.slice(-1) != '}')) sRet += JSON.stringify(value);else sRet += "'{" + JSON.stringify(value).replace(/'/g, "\\'") + "}'"; // replace '
  }
  ...
  if (iChildNum) {
    ...
    sRet += '>';
    if (sHtmlTxt) sRet += htmlEncode(sHtmlTxt);
  }

  if (iFlag == 3) sRet += '</span>\n';else sRet += '</div>\n';
  return sRet;
}
...
function childElements_(childEle, ownerWdgt) {
  ...
  if (child) {
    ...
    return bRet;
  } else return React.Children.toArray(childEle.props.children);
};
...
function getCompByPath_(entry, sPath_) {
  ...
  if (isSibling) {
    ...
    if (owner) return doCallback(sPath ? owner.W(sPath) : owner);
  } else if (iLevel == -1) {
...
function loadReference_(entry, keyName, callback) {
  // keyName must be string, callback must passed
  var isRefSpan = false,
      bSourProp = [],
      refLnkPath = '';
  var child,
  ...
  if (entry.widget && typeof compIdx == 'number' && keyName[0] == '$' && (child = entry.$gui.comps[compIdx]) && child.props['isReference.']) {
    ...
    if (typeof refLnkPath != 'string' || (refLnkPath = refLnkPath.trim()).length == 0) {
      ...
      if (!refLnkPath) doCallback(null, refLnkPath);else loadCompByRef(entry, refLnkPath, ''); // entry is parent component of a linker
    }
  } else {
    ...
    if (isSibling) {
      ...
      if (iLevel < 0) {
        ...
        if (targ && !sPath) {
          ...
          var targOwn = targ.parent,
              targOwnObj = targOwn && targOwn.component;
          if (targObj && targOwnObj) {
            var iPos = targOwnObj.$gui.compIdx[targObj.$gui.keyid];
            if (typeof iPos == 'number') return doCallback(targOwnObj.$gui.comps[iPos], sPath_);
          }
          ...
          if (iType == 2) {
            ...
            if (compProp2['hookTo.'] === tempNode && typeof sPath2_ == 'string') {
              ...
              if (ch != '/' && ch != '.') {
                ...
                if (sPath2_.indexOf('/') >= 0) return doCallback(null, sPath2_);
...
function pageCtrl_(bPage) {
  ...
  bPage.forEach(function (item, idx) {
    ...
    var page = item[1],
        dStyle = Object.assign({}, page.state.style);
...
pageCtrl_.prototype = {
  gotoPage: function gotoPage(pgIndex, callback) {
    ...
    if (pgIndex >= keys.length) pgIndex = keys.length - 1;
    ...
    keys.forEach(function (sKey, idx) {
      ...
      if (comp) {
        ...
        if (comp.state.style.display != sCss) comp.setState({ style: Object.assign({}, comp.state.style, { display: sCss }) });
      }
    });

    if (jumpedWdgt) // success
      this.pageIndex = pgIndex;else sLeftSeg = ''; // failed // sFirstSeg is ''
    doCallback();
  },
  ...
  setDisplay: function setDisplay(cfg) {
    if (cfg.hasOwnProperty('leftCtrlWidth')) this.leftPanel.style.width = getPxWidth(cfg.leftCtrlWidth) + 'px';
    if (cfg.hasOwnProperty('rightCtrlWidth')) this.rightPanel.style.width = getPxWidth(cfg.rightCtrlWidth) + 'px';
...
function listScenePage_() {
  ...
  if (topObj) {
    (topObj.$gui.comps || []).forEach(function (child) {
      ...
      childObj = childObj && childObj.component;
      if (childObj && childObj.props['isScenePage.']) bPage.push([sKey, childObj]);
    });
  }
  return bPage;
}
...
W.$main.$$onLoad_ = function (designCallback) {
  ...
  function processOneInit(onLoads) {
    var fn = onLoads.shift();
    if (fn) {
      fn(function () {
        processOneInit(onLoads);
      });
    } else {
      processRef(function () {
        justFirstRender_ = false;
        ...
        if (!W.__design__ && containNode_) {
          var bPage = listScenePage_();
          if (bPage.length) {
            // has ScenePage, need setup PageController
            if (utils.pageCtrl && utils.pageCtrl.destory) utils.pageCtrl.destory();
  ...
  processOneInit((W.$main.$$onLoad || []).slice(0));
};
...
function getSparedPixel_(obj, wdgt, isRow, iTotal) {
  ...
  for (var i = 0; i < iLen; i++) {
    ...
    if (childObj && childObj.state) // has mounted
      wd = isRow ? childObj.state.width : childObj.state.height;else wd = isRow ? child.props.width : child.props.height;
...
var TWidget_ = function () {
  ...
  _createClass2(TWidget_, [{
    ...
    value: function _getSchema(self, iLevel) {
      ...
      if (self && (owner = self.parentOf()) && owner.props['isTableRow.']) dRet.tdStyle = [iLevel + 12, 'object', null, '[object]: style for parent (td)'];
      return dRet;
    }
  }, {
    key: 'getDefaultProps',
    ...
    value: function getInitialState() {
      ...
      for (var i = 0, item; item = b[i]; i++) {
        if (item.indexOf('data-') == 0) {
          ...
          if (tp == 'string') {
            if (item == '$id__') {
              ...
              if (ctrlExprCmd_.indexOf(item) >= 0) {
                ...
                if (item != 'key') exprAttrs.push(item);
          ...
          if (tp == 'function') {
            ...
            item = item.slice(1);
            if (item == 'id__') {
              // pass setter by props.$id__
              dualIdSetter = itemValue; // will bind this in defineDual('id__')
              gui.hasIdSetter = true;
            } else if (!sysEvSet[item]) currEvSet[item] = itemValue.bind(this);
            // else, ignore
          }
      ...
      gui.compIdx = {};gui.comps = React.Children.toArray(this.props.children);
      ...
      if (owner && hookThis) {
        ...
        if (isTopmost && containNode_) {
          ...
          var refreshFrame = function () {
            ...
            if (this.duals.hasOwnProperty('innerSize')) {
          ...
          if (typeof containNode_.instantShow == 'function') W.$utils.instantShow = containNode_.instantShow; // overwrite old function
      ...
      this.defineDual('childNumId', function (newNumId, oldNumId) {
        ...
        bComp.forEach(function (child, iPos) {
          ...
          if (hasClass_(child.props.className, 'rewgt-static')) {
            ...
            if (!isPanelWdgt) {
              ...
              if (childInline) // div.rewgt-static --> span.rewgt-static
                bComp[iPos] = React.createElement('span', dProp);else bComp[iPos] = React.createElement('div', dProp); // React.cloneElement(child, dProp);
            } else {
          ...
          if (childInline2 !== undefined) {
            ...
            compIdx[keyid] = iPos;
            if (!isOld) bComp[iPos] = React.cloneElement(child, { 'keyid.': keyid, key: sKey });
            return;
          }
          ...
          if (isOld) // reuse old, no need cloneElement()
            compIdx[keyid] = iPos; // maybe position changed
              ...
              compIdx[keyid] = iPos;
              bComp[iPos] = React.cloneElement(child, props); // width/height should pass parseFloat() for scaning spared space
            }
        ...
        if (this.props['isNavigator.']) {
          ...
          for (var i = 0; i < compLen; i++) {
            ...
            if (child.props['isPlayground.']) bNavItems[child.props['keyid.']] = child; // must has 'keyid.'
    ...
    value: function componentDidMount() {
      ...
      var gui = this.$gui;
      if (!gui.isPanel && this.props['hasStatic.']) renewStaticChild(this);
    ...
    value: function reRender(callback) {
      // should not call reRender in self render()
      if (this.isHooked) this.setState({ id__: identicalId() }, callback);else {
        if (callback) callback();
      }
    }
  }, {
    ...
    value: function getHtmlNode(component) {
      return ReactDOM.findDOMNode(component || this);
    }
  }, {
    key: 'parentOf',
    value: function parentOf() {
    ...
    value: function fullClone(props) {
      ...
      if (ele) {
        if (iFlag = ele.props['isReference.']) {
          ...
          return React.createElement(iFlag == 1 ? RefDiv__ : RefSpan__, Object.assign(dProp, props));
        }

        if (this.props['isTemplate.'] && !W.__design__) {
          var tempObj = this.$gui.template;
          if (tempObj instanceof templateNode) return React.cloneElement(ele, { template: tempObj }, null);else throw new Error('invalid template (' + keyid + ')');
        }
      }
    ...
    value: function setChild() {
        ...
        if (justFirstRender_) {
          ...
          for (var i0 = 0; i0 < argLen; i0++) {
            ...
            if (argTp == 'function') {
              ...
              break;
            } else if (arg && React.isValidElement(arg)) setTemplateChild_(template, arg);
            // else, ignore
          }
      ...
      var isTopmost = thisWidget === topmostWidget_,
          currWidgetType = 4; // 1:panel, 2:unit, 3:paragraph, 4:inline
      ...
      if (changed) {
        ...
        for (var i = 0; i < iLen; i++) {
          ...
          if (Array.isArray(item)) {
            ...
            bComp[i] = React.cloneElement(item, props);
          } else {
    ...
    value: function prepareState() {
      ...
      function percentPx(parentWd, WD, canNeg) {
        var wd;
        if (WD >= 1) wd = WD;else if (typeof parentWd != 'number') return null;else if (WD >= 0.9999) wd = parentWd;else if (WD >= 0) wd = WD * parentWd;else wd = WD;

        if (wd < 0) {
          if (canNeg) {
            if (wd > -0.9999) return wd * parentWd;else if (wd > -1) return -parentWd;else return wd;
          } else return 0;
        } else return wd;
      }

      function toCssString(WD, perAsZero) {
        if (WD >= 1) return WD + 'px';else {
          if (perAsZero) return 0;
          if (WD > 0.9999) return '100%';else return WD * 100 + '%';
        }
      }
      ...
      var cssWd = null,
          wdAuto = typeof thisWd != 'number' ? true : thisWd >= 1 ? false : parentWdAuto;
      var cssHi = null,
          hiAuto = typeof thisHi != 'number' ? true : thisHi >= 1 ? false : parentHiAuto;
      var wdChanged = false,
          hiChanged = false;
      if (!wdAuto) {
        var wd = null;
      ...
      if (hiAuto) delete dStyle.height;else dStyle.height = cssHi + 'px';
...
var TPanel_ = function (_TWidget_2) {
  ...
  _createClass2(TPanel_, [{
    ...
    value: function getInitialState() {
      ...
      if (this.widget && this.widget.parent === topmostWidget_) {
        // direct under W.body
        if ((this.props.left || 0) > 0 || (this.props.top || 0) > 0) this.duals.style = { position: 'absolute' };
      }
      return dState;
    }
  }, {
            ...
            if (!gui.inDrag && !self.nextSibling() && (owner2_ = owner.parent) && (ownerObj2_ = owner2_.component)) {
              if (ownerObj.$gui.sparedTotal > 0.989 && ownerObj.$gui.sparedTotal <= 1.01) {
                ...
                if (!gui.inDrag && typeof wd == 'number' && wd > 0 && ownerObj2_.$gui.sparedTotal > 0.989 && ownerObj2_.$gui.sparedTotal <= 1.01) {
                  var owner3_ = owner2_.parent,
                      ownerObj3_ = owner3_ && owner3_.component;
                  if (ownerObj3_) {
...
var TGridPanel_ = function (_TPanel_) {
  ...
  _createClass2(TGridPanel_, [{
          ...
          gui.comps.forEach(function (child, iPos) {
            ...
            if (gui.inRow) {
              if (child.props.width !== wdHi) // wdHi maybe null
                gui.comps[iPos] = React.cloneElement(child, { width: wdHi });
            } else {
              if (child.props.height !== wdHi) gui.comps[iPos] = React.cloneElement(child, { height: wdHi });
            }
          });
        }
      });
    ...
    value: function render() {
      ...
      if (W.__design__) bChild.push(React.createElement('div', { key: '$end', title: 'end of GridPanel', style: { width: '100%', height: '10px', backgroundColor: '#eee' } }));
...
var TTableRow_ = function (_TUnit_2) {
  ...
  function TTableRow_(name, desc) {
    ...
    _this6._slientProp.push('isTableRow.'); // _statedProp no change
    _this6._defaultProp.height = undefined; // undefined stand for not a number, not save all none-number // careful: NaN === NaN is false
    return _this6;
  }
  ...
  _createClass2(TTableRow_, [{
    ...
    value: function _getSchema(self, iLevel) {
      iLevel = iLevel || 1200;
      return _get(TTableRow_.prototype.__proto__ || Object.getPrototypeOf(TTableRow_.prototype), '_getSchema', this).call(this, self, iLevel + 200);
    }
  }, {
    key: 'getDefaultProps',
    value: function getDefaultProps() {
    ...
    value: function render() {
      ...
      bComp.forEach(function (cell) {
        ...
        if (wdChanged && cellWd !== null || hiChanged) cell = React.cloneElement(cell, { width: cellWd, height: cellHi });

        bChild.push(React.createElement('td', dTdProp, cell));
      });
      ...
      return React.createElement('tr', props, bChild);
    }
  }]);
...
var TTablePanel_ = function (_TPanel_2) {
  ...
  function TTablePanel_(name, desc) {
    ...
    _this7._defaultProp.height = undefined;
    return _this7;
  }
...
var TDiv_ = function (_TUnit_3) {
  ...
  function TDiv_(name, desc) {
    ...
    _this8._slientProp.push('tagName.'); // _statedProp no change
    _this8._defaultProp.height = undefined;
  ...
  _createClass2(TDiv_, [{
    ...
    value: function getInitialState() {
      ...
      if (W.__debug__) {
        ...
        owner = owner && owner.parent;
        owner = owner && owner.component;
        if (owner && !hasClass_(owner.props.className, ['rewgt-panel', 'rewgt-unit']) && !owner.props['isScenePage.']) utils.instantShow('warning: ' + this._._className + ' should be used in panel widget or ScenePage');
      }
...
var TArticle_ = function (_TDiv_) {
  _inherits(TArticle_, _TDiv_);

  function TArticle_(name, desc) {
    _classCallCheck(this, TArticle_);

    return _possibleConstructorReturn(this, (TArticle_.__proto__ || Object.getPrototypeOf(TArticle_)).call(this, name || 'Article', desc));
  }

  _createClass2(TArticle_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TArticle_.prototype.__proto__ || Object.getPrototypeOf(TArticle_.prototype), 'getDefaultProps', this).call(this);
...
T.Article_ = TArticle_;
T.Article = new TArticle_();

var TSection_ = function (_TDiv_2) {
  _inherits(TSection_, _TDiv_2);

  function TSection_(name, desc) {
    _classCallCheck(this, TSection_);

    return _possibleConstructorReturn(this, (TSection_.__proto__ || Object.getPrototypeOf(TSection_)).call(this, name || 'Section', desc));
  }

  _createClass2(TSection_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TSection_.prototype.__proto__ || Object.getPrototypeOf(TSection_.prototype), 'getDefaultProps', this).call(this);
...
T.Section_ = TSection_;
T.Section = new TSection_();

var THeader_ = function (_TDiv_3) {
  _inherits(THeader_, _TDiv_3);

  function THeader_(name, desc) {
    _classCallCheck(this, THeader_);

    return _possibleConstructorReturn(this, (THeader_.__proto__ || Object.getPrototypeOf(THeader_)).call(this, name || 'Header', desc));
  }

  _createClass2(THeader_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(THeader_.prototype.__proto__ || Object.getPrototypeOf(THeader_.prototype), 'getDefaultProps', this).call(this);
...
T.Header_ = THeader_;
T.Header = new THeader_();

var TFooter_ = function (_TDiv_4) {
  _inherits(TFooter_, _TDiv_4);

  function TFooter_(name, desc) {
    _classCallCheck(this, TFooter_);

    return _possibleConstructorReturn(this, (TFooter_.__proto__ || Object.getPrototypeOf(TFooter_)).call(this, name || 'Footer', desc));
  }

  _createClass2(TFooter_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TFooter_.prototype.__proto__ || Object.getPrototypeOf(TFooter_.prototype), 'getDefaultProps', this).call(this);
...
T.Footer_ = TFooter_;
T.Footer = new TFooter_();

var TAside_ = function (_TDiv_5) {
  _inherits(TAside_, _TDiv_5);

  function TAside_(name, desc) {
    _classCallCheck(this, TAside_);

    return _possibleConstructorReturn(this, (TAside_.__proto__ || Object.getPrototypeOf(TAside_)).call(this, name || 'Aside', desc));
  }

  _createClass2(TAside_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TAside_.prototype.__proto__ || Object.getPrototypeOf(TAside_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'aside';
      return props;
    }
  }]);
...
var TNav_ = function (_TDiv_6) {
  _inherits(TNav_, _TDiv_6);

  function TNav_(name, desc) {
    _classCallCheck(this, TNav_);

    return _possibleConstructorReturn(this, (TNav_.__proto__ || Object.getPrototypeOf(TNav_)).call(this, name || 'Nav', desc));
  }
...
var TMain_ = function (_TDiv_7) {
  _inherits(TMain_, _TDiv_7);

  function TMain_(name, desc) {
    _classCallCheck(this, TMain_);

    return _possibleConstructorReturn(this, (TMain_.__proto__ || Object.getPrototypeOf(TMain_)).call(this, name || 'Main', desc));
  }

  _createClass2(TMain_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TMain_.prototype.__proto__ || Object.getPrototypeOf(TMain_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'main';
      return props;
    }
  }]);
...
var TP_ = function (_TUnit_4) {
  ...
  function TP_(name, desc) {
    ...
    var _this16 = _possibleConstructorReturn(this, (TP_.__proto__ || Object.getPrototypeOf(TP_)).call(this, name || 'P', desc));

    _this16._slientProp.push('tagName.'); // _statedProp no change
    _this16._defaultProp.width = undefined;
    _this16._defaultProp.height = undefined;
    _this16._defaultProp.margin = TPara_margin_.slice(0);
    return _this16;
  }
...
var TNoscript_ = function (_TP_) {
  ...
  function TNoscript_(name, desc) {
    ...
    return _possibleConstructorReturn(this, (TNoscript_.__proto__ || Object.getPrototypeOf(TNoscript_)).call(this, name || 'Noscript', desc));
  }
...
var TFieldset_ = function (_TP_2) {
  ...
  function TFieldset_(name, desc) {
    ...
    var _this18 = _possibleConstructorReturn(this, (TFieldset_.__proto__ || Object.getPrototypeOf(TFieldset_)).call(this, name || 'Fieldset', desc));

    _this18._defaultProp.padding = TFieldset_padding_.slice(0);
    _this18._defaultProp.borderWidth = TFieldset_border_width_.slice(0);
    return _this18;
  }
...
var TDetails_ = function (_TP_3) {
  _inherits(TDetails_, _TP_3);

  function TDetails_(name, desc) {
    _classCallCheck(this, TDetails_);

    return _possibleConstructorReturn(this, (TDetails_.__proto__ || Object.getPrototypeOf(TDetails_)).call(this, name || 'Details', desc));
  }

  _createClass2(TDetails_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TDetails_.prototype.__proto__ || Object.getPrototypeOf(TDetails_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'details';
      return props;
    }
  }]);
...
T.Details_ = TDetails_;
T.Details = new TDetails_();

var TUl_padding_ = [0, 0, 0, 20];

var TUl_ = function (_TP_4) {
  _inherits(TUl_, _TP_4);

  function TUl_(name, desc) {
    _classCallCheck(this, TUl_);

    var _this20 = _possibleConstructorReturn(this, (TUl_.__proto__ || Object.getPrototypeOf(TUl_)).call(this, name || 'Ul', desc));

    _this20._defaultProp.padding = TUl_padding_.slice(0); // _statedProp, _slientProp no change
    return _this20;
  }
...
var TOl_ = function (_TP_5) {
  _inherits(TOl_, _TP_5);

  function TOl_(name, desc) {
    _classCallCheck(this, TOl_);

    var _this21 = _possibleConstructorReturn(this, (TOl_.__proto__ || Object.getPrototypeOf(TOl_)).call(this, name || 'Ol', desc));

    _this21._defaultProp.padding = TUl_padding_.slice(0); // _statedProp, _slientProp no change
    return _this21;
  }
...
var TLi_ = function (_TP_6) {
  _inherits(TLi_, _TP_6);

  function TLi_(name, desc) {
    _classCallCheck(this, TLi_);

    return _possibleConstructorReturn(this, (TLi_.__proto__ || Object.getPrototypeOf(TLi_)).call(this, name || 'Li', desc));
  }

  _createClass2(TLi_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TLi_.prototype.__proto__ || Object.getPrototypeOf(TLi_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'li';
      return props;
    }
  }]);
...
var TDl_ = function (_TP_7) {
  _inherits(TDl_, _TP_7);

  function TDl_(name, desc) {
    _classCallCheck(this, TDl_);

    return _possibleConstructorReturn(this, (TDl_.__proto__ || Object.getPrototypeOf(TDl_)).call(this, name || 'Dl', desc));
  }

  _createClass2(TDl_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TDl_.prototype.__proto__ || Object.getPrototypeOf(TDl_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'dl';
      return props;
    }
  }]);
...
var TDd_ = function (_TP_8) {
  _inherits(TDd_, _TP_8);

  function TDd_(name, desc) {
    _classCallCheck(this, TDd_);

    return _possibleConstructorReturn(this, (TDd_.__proto__ || Object.getPrototypeOf(TDd_)).call(this, name || 'Dd', desc));
  }

  _createClass2(TDd_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TDd_.prototype.__proto__ || Object.getPrototypeOf(TDd_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'dd';
      return props;
    }
  }]);
...
var TDt_ = function (_TP_9) {
  _inherits(TDt_, _TP_9);

  function TDt_(name, desc) {
    _classCallCheck(this, TDt_);

    return _possibleConstructorReturn(this, (TDt_.__proto__ || Object.getPrototypeOf(TDt_)).call(this, name || 'Dt', desc));
  }

  _createClass2(TDt_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TDt_.prototype.__proto__ || Object.getPrototypeOf(TDt_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'dt';
      return props;
    }
  }]);
...
var TFigure_ = function (_TP_10) {
  _inherits(TFigure_, _TP_10);

  function TFigure_(name, desc) {
    _classCallCheck(this, TFigure_);

    return _possibleConstructorReturn(this, (TFigure_.__proto__ || Object.getPrototypeOf(TFigure_)).call(this, name || 'Figure', desc));
  }

  _createClass2(TFigure_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TFigure_.prototype.__proto__ || Object.getPrototypeOf(TFigure_.prototype), 'getDefaultProps', this).call(this);
...
T.Figure_ = TFigure_;
T.Figure = new TFigure_();

var TFigcaption_ = function (_TP_11) {
  _inherits(TFigcaption_, _TP_11);

  function TFigcaption_(name, desc) {
    _classCallCheck(this, TFigcaption_);

    return _possibleConstructorReturn(this, (TFigcaption_.__proto__ || Object.getPrototypeOf(TFigcaption_)).call(this, name || 'Figcaption', desc));
  }

  _createClass2(TFigcaption_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TFigcaption_.prototype.__proto__ || Object.getPrototypeOf(TFigcaption_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'figcaption';
      return props;
    }
  }]);
...
T.Figcaption_ = TFigcaption_;
T.Figcaption = new TFigcaption_();

var TMenu_ = function (_TP_12) {
  _inherits(TMenu_, _TP_12);

  function TMenu_(name, desc) {
    _classCallCheck(this, TMenu_);

    return _possibleConstructorReturn(this, (TMenu_.__proto__ || Object.getPrototypeOf(TMenu_)).call(this, name || 'Menu', desc));
  }

  _createClass2(TMenu_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TMenu_.prototype.__proto__ || Object.getPrototypeOf(TMenu_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'menu';
      return props;
    }
  }]);
...
var TMenuitem_ = function (_TP_13) {
  _inherits(TMenuitem_, _TP_13);

  function TMenuitem_(name, desc) {
    _classCallCheck(this, TMenuitem_);

    return _possibleConstructorReturn(this, (TMenuitem_.__proto__ || Object.getPrototypeOf(TMenuitem_)).call(this, name || 'Menuitem', desc));
  }

  _createClass2(TMenuitem_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TMenuitem_.prototype.__proto__ || Object.getPrototypeOf(TMenuitem_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'menuitem';
      return props;
    }
  }]);
...
T.Menuitem_ = TMenuitem_;
T.Menuitem = new TMenuitem_();

var TAddress_ = function (_TP_14) {
  _inherits(TAddress_, _TP_14);

  function TAddress_(name, desc) {
    _classCallCheck(this, TAddress_);

    return _possibleConstructorReturn(this, (TAddress_.__proto__ || Object.getPrototypeOf(TAddress_)).call(this, name || 'Address', desc));
  }

  _createClass2(TAddress_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TAddress_.prototype.__proto__ || Object.getPrototypeOf(TAddress_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'address';
      return props;
    }
  }]);
...
T.Address_ = TAddress_;
T.Address = new TAddress_();

var TForm_ = function (_TP_15) {
  _inherits(TForm_, _TP_15);

  function TForm_(name, desc) {
    _classCallCheck(this, TForm_);

    return _possibleConstructorReturn(this, (TForm_.__proto__ || Object.getPrototypeOf(TForm_)).call(this, name || 'Form', desc));
  }

  _createClass2(TForm_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
...
var TIframe_ = function (_TP_16) {
  _inherits(TIframe_, _TP_16);

  function TIframe_(name, desc) {
    _classCallCheck(this, TIframe_);

    var _this32 = _possibleConstructorReturn(this, (TIframe_.__proto__ || Object.getPrototypeOf(TIframe_)).call(this, name || 'Iframe', desc));

    _this32._defaultProp.margin = TIframe_margin_.slice(0);
    return _this32;
  }
...
var TVideo_ = function (_TP_17) {
  _inherits(TVideo_, _TP_17);
...
var TCanvas_ = function (_TP_18) {
  _inherits(TCanvas_, _TP_18);

  function TCanvas_(name, desc) {
    _classCallCheck(this, TCanvas_);

    return _possibleConstructorReturn(this, (TCanvas_.__proto__ || Object.getPrototypeOf(TCanvas_)).call(this, name || 'Canvas', desc));
  }

  _createClass2(TCanvas_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TCanvas_.prototype.__proto__ || Object.getPrototypeOf(TCanvas_.prototype), 'getDefaultProps', this).call(this);
...
var TBlockquote_ = function (_TP_19) {
  _inherits(TBlockquote_, _TP_19);

  function TBlockquote_(name, desc) {
    _classCallCheck(this, TBlockquote_);

    var _this35 = _possibleConstructorReturn(this, (TBlockquote_.__proto__ || Object.getPrototypeOf(TBlockquote_)).call(this, name || 'Blockquote', desc));

    _this35._defaultProp.padding = TBlockquote_padding_.slice(0);
    return _this35;
  }
...
var TTable_ = function (_TP_20) {
  _inherits(TTable_, _TP_20);

  function TTable_(name, desc) {
    _classCallCheck(this, TTable_);

    return _possibleConstructorReturn(this, (TTable_.__proto__ || Object.getPrototypeOf(TTable_)).call(this, name || 'Table', desc));
  }
...
var TCaption_ = function (_TP_21) {
  _inherits(TCaption_, _TP_21);

  function TCaption_(name, desc) {
    _classCallCheck(this, TCaption_);

    return _possibleConstructorReturn(this, (TCaption_.__proto__ || Object.getPrototypeOf(TCaption_)).call(this, name || 'Caption', desc));
  }

  _createClass2(TCaption_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TCaption_.prototype.__proto__ || Object.getPrototypeOf(TCaption_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'caption';
      return props;
    }
  }]);
...
T.Caption_ = TCaption_;
T.Caption = new TCaption_();

var TCol_ = function (_TP_22) {
  _inherits(TCol_, _TP_22);

  function TCol_(name, desc) {
    _classCallCheck(this, TCol_);

    return _possibleConstructorReturn(this, (TCol_.__proto__ || Object.getPrototypeOf(TCol_)).call(this, name || 'Col', desc));
  }
...
var TColgroup_ = function (_TP_23) {
  _inherits(TColgroup_, _TP_23);

  function TColgroup_(name, desc) {
    _classCallCheck(this, TColgroup_);

    return _possibleConstructorReturn(this, (TColgroup_.__proto__ || Object.getPrototypeOf(TColgroup_)).call(this, name || 'Colgroup', desc));
  }

  _createClass2(TColgroup_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TColgroup_.prototype.__proto__ || Object.getPrototypeOf(TColgroup_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'colgroup';
      return props;
    }
  }]);
...
T.Colgroup_ = TColgroup_;
T.Colgroup = new TColgroup_();

var TTd_ = function (_TP_24) {
  _inherits(TTd_, _TP_24);

  function TTd_(name, desc) {
    _classCallCheck(this, TTd_);

    return _possibleConstructorReturn(this, (TTd_.__proto__ || Object.getPrototypeOf(TTd_)).call(this, name || 'Td', desc));
  }

  _createClass2(TTd_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TTd_.prototype.__proto__ || Object.getPrototypeOf(TTd_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'td';
      return props;
    }
  }]);
...
var TTbody_ = function (_TP_25) {
  _inherits(TTbody_, _TP_25);

  function TTbody_(name, desc) {
    _classCallCheck(this, TTbody_);

    var _this41 = _possibleConstructorReturn(this, (TTbody_.__proto__ || Object.getPrototypeOf(TTbody_)).call(this, name || 'Tbody', desc));

    _this41._defaultProp.margin = TBody_margin_.slice(0);
    return _this41;
  }
  ...
  _createClass2(TTbody_, [{
    ...
    value: function render() {
      if (W.__design__) {
        ...
        var bChild = this.prepareState();
        var tailRow = React.createElement('tr', { key: '$end', title: 'end of Tbody', style: { height: '10px' } }, React.createElement('td'));
        bChild.push(tailRow);
...
var TThead_ = function (_TP_26) {
  _inherits(TThead_, _TP_26);

  function TThead_(name, desc) {
    _classCallCheck(this, TThead_);

    return _possibleConstructorReturn(this, (TThead_.__proto__ || Object.getPrototypeOf(TThead_)).call(this, name || 'Thead', desc));
  }

  _createClass2(TThead_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TThead_.prototype.__proto__ || Object.getPrototypeOf(TThead_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'thead';
      return props;
    }
  }]);
...
var TTfoot_ = function (_TP_27) {
  _inherits(TTfoot_, _TP_27);

  function TTfoot_(name, desc) {
    _classCallCheck(this, TTfoot_);

    return _possibleConstructorReturn(this, (TTfoot_.__proto__ || Object.getPrototypeOf(TTfoot_)).call(this, name || 'Tfoot', desc));
  }

  _createClass2(TTfoot_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TTfoot_.prototype.__proto__ || Object.getPrototypeOf(TTfoot_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'tfoot';
      return props;
    }
  }]);
...
var TTh_ = function (_TP_28) {
  _inherits(TTh_, _TP_28);

  function TTh_(name, desc) {
    _classCallCheck(this, TTh_);

    var _this44 = _possibleConstructorReturn(this, (TTh_.__proto__ || Object.getPrototypeOf(TTh_)).call(this, name || 'Th', desc));

    _this44._defaultProp.margin = ThTr_margin_.slice(0);
    return _this44;
  }
  ...
  _createClass2(TTh_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TTh_.prototype.__proto__ || Object.getPrototypeOf(TTh_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'th';
...
var TTr_ = function (_TP_29) {
  _inherits(TTr_, _TP_29);

  function TTr_(name, desc) {
    _classCallCheck(this, TTr_);

    var _this45 = _possibleConstructorReturn(this, (TTr_.__proto__ || Object.getPrototypeOf(TTr_)).call(this, name || 'Tr', desc));

    _this45._defaultProp.margin = ThTr_margin_.slice(0);
    return _this45;
  }
  ...
  _createClass2(TTr_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TTr_.prototype.__proto__ || Object.getPrototypeOf(TTr_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'tr';
      props.margin = ThTr_margin_.slice(0);
      return props;
    }
...
var THgroup_ = function (_TP_30) {
  _inherits(THgroup_, _TP_30);

  function THgroup_(name, desc) {
    _classCallCheck(this, THgroup_);

    return _possibleConstructorReturn(this, (THgroup_.__proto__ || Object.getPrototypeOf(THgroup_)).call(this, name || 'Hgroup', desc));
  }

  _createClass2(THgroup_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(THgroup_.prototype.__proto__ || Object.getPrototypeOf(THgroup_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'hgroup';
      return props;
    }
  }]);
...
T.Hgroup_ = THgroup_;
T.Hgroup = new THgroup_();

var TH1_ = function (_TP_31) {
  _inherits(TH1_, _TP_31);

  function TH1_(name, desc) {
    _classCallCheck(this, TH1_);

    return _possibleConstructorReturn(this, (TH1_.__proto__ || Object.getPrototypeOf(TH1_)).call(this, name || 'H1', desc));
  }

  _createClass2(TH1_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TH1_.prototype.__proto__ || Object.getPrototypeOf(TH1_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'h1';
      return props;
    }
  }]);
...
var TH2_ = function (_TP_32) {
  _inherits(TH2_, _TP_32);

  function TH2_(name, desc) {
    _classCallCheck(this, TH2_);

    return _possibleConstructorReturn(this, (TH2_.__proto__ || Object.getPrototypeOf(TH2_)).call(this, name || 'H2', desc));
  }

  _createClass2(TH2_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TH2_.prototype.__proto__ || Object.getPrototypeOf(TH2_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'h2';
      return props;
    }
  }]);
...
var TH3_ = function (_TP_33) {
  _inherits(TH3_, _TP_33);

  function TH3_(name, desc) {
    _classCallCheck(this, TH3_);

    return _possibleConstructorReturn(this, (TH3_.__proto__ || Object.getPrototypeOf(TH3_)).call(this, name || 'H3', desc));
  }

  _createClass2(TH3_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TH3_.prototype.__proto__ || Object.getPrototypeOf(TH3_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'h3';
      return props;
    }
  }]);
...
var TH4_ = function (_TP_34) {
  _inherits(TH4_, _TP_34);

  function TH4_(name, desc) {
    _classCallCheck(this, TH4_);

    return _possibleConstructorReturn(this, (TH4_.__proto__ || Object.getPrototypeOf(TH4_)).call(this, name || 'H4', desc));
  }

  _createClass2(TH4_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TH4_.prototype.__proto__ || Object.getPrototypeOf(TH4_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'h4';
      return props;
    }
  }]);
...
var TH5_ = function (_TP_35) {
  _inherits(TH5_, _TP_35);

  function TH5_(name, desc) {
    _classCallCheck(this, TH5_);

    return _possibleConstructorReturn(this, (TH5_.__proto__ || Object.getPrototypeOf(TH5_)).call(this, name || 'H5', desc));
  }

  _createClass2(TH5_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TH5_.prototype.__proto__ || Object.getPrototypeOf(TH5_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'h5';
      return props;
    }
  }]);
...
var TH6_ = function (_TP_36) {
  _inherits(TH6_, _TP_36);

  function TH6_(name, desc) {
    _classCallCheck(this, TH6_);

    return _possibleConstructorReturn(this, (TH6_.__proto__ || Object.getPrototypeOf(TH6_)).call(this, name || 'H6', desc));
  }

  _createClass2(TH6_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TH6_.prototype.__proto__ || Object.getPrototypeOf(TH6_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'h6';
      return props;
    }
  }]);
...
var THr_ = function (_TP_37) {
  _inherits(THr_, _TP_37);

  function THr_(name, desc) {
    _classCallCheck(this, THr_);

    var _this53 = _possibleConstructorReturn(this, (THr_.__proto__ || Object.getPrototypeOf(THr_)).call(this, name || 'Hr', desc));

    _this53._defaultProp.width = 0.9999;
    _this53._defaultProp.borderWidth = THr_border_width_.slice(0);
    return _this53;
  }
...
var TPre_ = function (_TP_38) {
  _inherits(TPre_, _TP_38);
...
var TSpan_ = function (_TWidget_4) {
  ...
  function TSpan_(name, desc) {
    ...
    var _this55 = _possibleConstructorReturn(this, (TSpan_.__proto__ || Object.getPrototypeOf(TSpan_)).call(this, name || 'Span', desc));

    _this55._statedProp = [];
    _this55._slientProp = ['className', 'hookTo.', 'keyid.', 'childInline.', 'tagName.'];
    _this55._defaultProp = { width: undefined, height: undefined }; // 'html.' default is not passed, suggest not use width/height, but if used also OK
    return _this55;
  }
  ...
  _createClass2(TSpan_, [{
    ...
    value: function getInitialState() {
      ...
      this.defineDual('childNumId', function (newNumId, oldNumId) {
        ...
        var compIdx = gui.compIdx,
            bComp = gui.comps;
        bComp.forEach(function (child, iPos) {
          if (typeof child == 'string') child = bComp[iPos] = React.createElement(Span__, { 'html.': child }); // auto change to React Element
          else if (!child) return;
          ...
          if (hasClass_(child.props.className, 'rewgt-static')) {
            ...
            compIdx[keyid] = iPos;
            bComp[iPos] = React.createElement('span', dProp); // div.rewgt-static --> span.rewgt-static
            return; // adjust nothing on rewgt-static widget
          }
          ...
          compIdx[keyid] = iPos;
          if (!isOld) bComp[iPos] = React.cloneElement(child, { 'hookTo.': hookThis, key: sKey, 'keyid.': keyid });
        });
      });
    ...
    value: function render() {
      syncProps_(this);
      if (this.hideThis) return React.createElement('span', displayNoneProp_);

      var props = setupRenderProp_(this);
      var b = this.$gui.comps,
          children = b.length ? b : this.state['html.'];
      return React.createElement(this.props['tagName.'], props, children);
    }
  }]);
...
var TBr_ = function (_TSpan_) {
  _inherits(TBr_, _TSpan_);

  function TBr_(name, desc) {
    _classCallCheck(this, TBr_);

    return _possibleConstructorReturn(this, (TBr_.__proto__ || Object.getPrototypeOf(TBr_)).call(this, name || 'Br', desc));
  }
...
var TA_ = function (_TSpan_2) {
  _inherits(TA_, _TSpan_2);
...
var TQ_ = function (_TSpan_3) {
  _inherits(TQ_, _TSpan_3);

  function TQ_(name, desc) {
    _classCallCheck(this, TQ_);

    return _possibleConstructorReturn(this, (TQ_.__proto__ || Object.getPrototypeOf(TQ_)).call(this, name || 'Q', desc));
  }

  _createClass2(TQ_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TQ_.prototype.__proto__ || Object.getPrototypeOf(TQ_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'q';
      return props;
    }
  }]);
...
var TAbbr_ = function (_TSpan_4) {
  _inherits(TAbbr_, _TSpan_4);

  function TAbbr_(name, desc) {
    _classCallCheck(this, TAbbr_);

    return _possibleConstructorReturn(this, (TAbbr_.__proto__ || Object.getPrototypeOf(TAbbr_)).call(this, name || 'Abbr', desc));
  }

  _createClass2(TAbbr_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TAbbr_.prototype.__proto__ || Object.getPrototypeOf(TAbbr_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'abbr';
      return props;
    }
  }]);
...
var TSource_ = function (_TSpan_6) {
  _inherits(TSource_, _TSpan_6);

  function TSource_(name, desc) {
    _classCallCheck(this, TSource_);

    return _possibleConstructorReturn(this, (TSource_.__proto__ || Object.getPrototypeOf(TSource_)).call(this, name || 'Source', desc));
  }

  _createClass2(TSource_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TSource_.prototype.__proto__ || Object.getPrototypeOf(TSource_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'source';
      return props;
    }
  }]);
...
T.Source_ = TSource_;
T.Source = new TSource_();

var TTrack_ = function (_TSpan_7) {
  _inherits(TTrack_, _TSpan_7);

  function TTrack_(name, desc) {
    _classCallCheck(this, TTrack_);

    return _possibleConstructorReturn(this, (TTrack_.__proto__ || Object.getPrototypeOf(TTrack_)).call(this, name || 'Track', desc));
  }

  _createClass2(TTrack_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TTrack_.prototype.__proto__ || Object.getPrototypeOf(TTrack_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'track';
      return props;
    }
  }]);
...
var TBdi_ = function (_TSpan_8) {
  _inherits(TBdi_, _TSpan_8);

  function TBdi_(name, desc) {
    _classCallCheck(this, TBdi_);

    return _possibleConstructorReturn(this, (TBdi_.__proto__ || Object.getPrototypeOf(TBdi_)).call(this, name || 'Bdi', desc));
  }

  _createClass2(TBdi_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TBdi_.prototype.__proto__ || Object.getPrototypeOf(TBdi_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'bdi';
      return props;
    }
  }]);
...
var TBdo_ = function (_TSpan_9) {
  _inherits(TBdo_, _TSpan_9);

  function TBdo_(name, desc) {
    _classCallCheck(this, TBdo_);

    return _possibleConstructorReturn(this, (TBdo_.__proto__ || Object.getPrototypeOf(TBdo_)).call(this, name || 'Bdo', desc));
  }

  _createClass2(TBdo_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TBdo_.prototype.__proto__ || Object.getPrototypeOf(TBdo_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'bdo';
      return props;
    }
  }]);
...
var TData_ = function (_TSpan_10) {
  _inherits(TData_, _TSpan_10);

  function TData_(name, desc) {
    _classCallCheck(this, TData_);

    return _possibleConstructorReturn(this, (TData_.__proto__ || Object.getPrototypeOf(TData_)).call(this, name || 'Data', desc));
  }

  _createClass2(TData_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TData_.prototype.__proto__ || Object.getPrototypeOf(TData_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'data';
      return props;
    }
  }]);
...
var TMark_ = function (_TSpan_11) {
  _inherits(TMark_, _TSpan_11);

  function TMark_(name, desc) {
    _classCallCheck(this, TMark_);

    return _possibleConstructorReturn(this, (TMark_.__proto__ || Object.getPrototypeOf(TMark_)).call(this, name || 'Mark', desc));
  }

  _createClass2(TMark_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TMark_.prototype.__proto__ || Object.getPrototypeOf(TMark_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'mark';
      return props;
    }
  }]);
...
var TWbr_ = function (_TSpan_12) {
  _inherits(TWbr_, _TSpan_12);

  function TWbr_(name, desc) {
    _classCallCheck(this, TWbr_);

    return _possibleConstructorReturn(this, (TWbr_.__proto__ || Object.getPrototypeOf(TWbr_)).call(this, name || 'Wbr', desc));
  }

  _createClass2(TWbr_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TWbr_.prototype.__proto__ || Object.getPrototypeOf(TWbr_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'wbr';
      return props;
    }
  }]);
...
var TButton_ = function (_TSpan_13) {
  _inherits(TButton_, _TSpan_13);

  function TButton_(name, desc) {
    _classCallCheck(this, TButton_);

    return _possibleConstructorReturn(this, (TButton_.__proto__ || Object.getPrototypeOf(TButton_)).call(this, name || 'Button', desc));
  }

  _createClass2(TButton_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TButton_.prototype.__proto__ || Object.getPrototypeOf(TButton_.prototype), 'getDefaultProps', this).call(this);
...
T.Button_ = TButton_;
T.Button = new TButton_();

var TTextarea_ = function (_TSpan_14) {
  _inherits(TTextarea_, _TSpan_14);
...
var TProgress_ = function (_TSpan_15) {
  _inherits(TProgress_, _TSpan_15);
...
var TImg_ = function (_TSpan_16) {
  _inherits(TImg_, _TSpan_16);

  function TImg_(name, desc) {
    _classCallCheck(this, TImg_);

    return _possibleConstructorReturn(this, (TImg_.__proto__ || Object.getPrototypeOf(TImg_)).call(this, name || 'Img', desc));
  }
...
var TPicture_ = function (_TSpan_17) {
  _inherits(TPicture_, _TSpan_17);

  function TPicture_(name, desc) {
    _classCallCheck(this, TPicture_);

    return _possibleConstructorReturn(this, (TPicture_.__proto__ || Object.getPrototypeOf(TPicture_)).call(this, name || 'Picture', desc));
  }

  _createClass2(TPicture_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TPicture_.prototype.__proto__ || Object.getPrototypeOf(TPicture_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'picture';
      return props;
    }
  }]);
...
T.Picture_ = TPicture_;
T.Picture = new TPicture_();

var TMap_ = function (_TSpan_18) {
  _inherits(TMap_, _TSpan_18);

  function TMap_(name, desc) {
    _classCallCheck(this, TMap_);

    return _possibleConstructorReturn(this, (TMap_.__proto__ || Object.getPrototypeOf(TMap_)).call(this, name || 'Map', desc));
  }

  _createClass2(TMap_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TMap_.prototype.__proto__ || Object.getPrototypeOf(TMap_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'map';
      return props;
    }
  }]);
...
var TArea_ = function (_TSpan_19) {
  _inherits(TArea_, _TSpan_19);

  function TArea_(name, desc) {
    _classCallCheck(this, TArea_);

    return _possibleConstructorReturn(this, (TArea_.__proto__ || Object.getPrototypeOf(TArea_)).call(this, name || 'Area', desc));
  }

  _createClass2(TArea_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TArea_.prototype.__proto__ || Object.getPrototypeOf(TArea_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'area';
      return props;
    }
  }]);
...
var TTime_ = function (_TSpan_20) {
  _inherits(TTime_, _TSpan_20);

  function TTime_(name, desc) {
    _classCallCheck(this, TTime_);

    return _possibleConstructorReturn(this, (TTime_.__proto__ || Object.getPrototypeOf(TTime_)).call(this, name || 'Time', desc));
  }

  _createClass2(TTime_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TTime_.prototype.__proto__ || Object.getPrototypeOf(TTime_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'time';
      return props;
    }
  }]);
...
var TOutput_ = function (_TSpan_21) {
  _inherits(TOutput_, _TSpan_21);

  function TOutput_(name, desc) {
    _classCallCheck(this, TOutput_);

    return _possibleConstructorReturn(this, (TOutput_.__proto__ || Object.getPrototypeOf(TOutput_)).call(this, name || 'Output', desc));
  }

  _createClass2(TOutput_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TOutput_.prototype.__proto__ || Object.getPrototypeOf(TOutput_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'output';
      return props;
    }
  }]);
...
T.Output_ = TOutput_;
T.Output = new TOutput_();

var TInput_ = function (_TSpan_22) {
  _inherits(TInput_, _TSpan_22);

  function TInput_(name, desc) {
    _classCallCheck(this, TInput_);

    return _possibleConstructorReturn(this, (TInput_.__proto__ || Object.getPrototypeOf(TInput_)).call(this, name || 'Input', desc));
  }
...
var TKeygen_ = function (_TSpan_23) {
  _inherits(TKeygen_, _TSpan_23);

  function TKeygen_(name, desc) {
    _classCallCheck(this, TKeygen_);

    return _possibleConstructorReturn(this, (TKeygen_.__proto__ || Object.getPrototypeOf(TKeygen_)).call(this, name || 'Keygen', desc));
  }

  _createClass2(TKeygen_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TKeygen_.prototype.__proto__ || Object.getPrototypeOf(TKeygen_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'keygen';
      return props;
    }
  }]);
...
T.Keygen_ = TKeygen_;
T.Keygen = new TKeygen_();

var TLabel_ = function (_TSpan_24) {
  _inherits(TLabel_, _TSpan_24);
...
var TLegend_ = function (_TSpan_25) {
  _inherits(TLegend_, _TSpan_25);

  function TLegend_(name, desc) {
    _classCallCheck(this, TLegend_);

    return _possibleConstructorReturn(this, (TLegend_.__proto__ || Object.getPrototypeOf(TLegend_)).call(this, name || 'Legend', desc));
  }

  _createClass2(TLegend_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TLegend_.prototype.__proto__ || Object.getPrototypeOf(TLegend_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'legend';
      return props;
    }
  }]);
...
T.Legend_ = TLegend_;
T.Legend = new TLegend_();

var TSub_ = function (_TSpan_26) {
  _inherits(TSub_, _TSpan_26);

  function TSub_(name, desc) {
    _classCallCheck(this, TSub_);

    return _possibleConstructorReturn(this, (TSub_.__proto__ || Object.getPrototypeOf(TSub_)).call(this, name || 'Sub', desc));
  }

  _createClass2(TSub_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TSub_.prototype.__proto__ || Object.getPrototypeOf(TSub_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'sub';
      return props;
    }
  }]);
...
var TSup_ = function (_TSpan_27) {
  _inherits(TSup_, _TSpan_27);

  function TSup_(name, desc) {
    _classCallCheck(this, TSup_);

    return _possibleConstructorReturn(this, (TSup_.__proto__ || Object.getPrototypeOf(TSup_)).call(this, name || 'Sup', desc));
  }

  _createClass2(TSup_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TSup_.prototype.__proto__ || Object.getPrototypeOf(TSup_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'sup';
      return props;
    }
  }]);
...
var TSelect_ = function (_TSpan_28) {
  _inherits(TSelect_, _TSpan_28);
  ...
  _createClass2(TSelect_, [{
    ...
    key: 'syncSelected',
    value: function syncSelected(thisNode) {
      if (!thisNode) thisNode = ReactDOM.findDOMNode(this);
      if (!thisNode) return;

      var bComp_ = this.$gui.comps,
          owner = this.widget;
      if (owner) {
        var selects = [],
            opts = thisNode.options;
        ...
        bComp_.forEach(function (child) {
          ...
          if (sName == 'option') {
            ...
            var sKey = getElementKey_(child);
            if (!sKey) return;
            var bChild = childElements_(child, owner),
                sPrefix = sKey + '.';
            bChild.forEach(function (child2) {
              if (child2.props['tagName.'] == 'option') {
                var sKey2 = getElementKey_(child2);
                if (sKey2) bComp.push([sPrefix + sKey2, child2]);
              }
            });
          }
        });

        bComp.forEach(function (bChild, idx) {
          // we not use 'event.target.selectedIndex' since we scan all <option>
          var sSeg = bChild[0],
              child = bChild[1];
          var childObj = owner.W(sSeg);
          childObj = childObj && childObj.component;
          if (childObj) {
            var selected = selects[idx],
                checked = childObj.state['data-checked'];
            if (typeof checked == 'string') {
              if (!selected && checked) {
                if (childObj.duals.hasOwnProperty('data-checked')) childObj.duals['data-checked'] = '';
              } else if (selected && !checked) {
                if (childObj.setChecked) childObj.setChecked(null); // callback=null, force=false
                else {
                    if (childObj.duals.hasOwnProperty('data-checked')) childObj.duals['data-checked'] = '1';
                  }
              }
            }
          }
...
var TDatalist_ = function (_TSelect_) {
  _inherits(TDatalist_, _TSelect_);

  function TDatalist_(name, desc) {
    _classCallCheck(this, TDatalist_);

    return _possibleConstructorReturn(this, (TDatalist_.__proto__ || Object.getPrototypeOf(TDatalist_)).call(this, name || 'Datalist', desc));
  }

  _createClass2(TDatalist_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TDatalist_.prototype.__proto__ || Object.getPrototypeOf(TDatalist_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'datalist';
      return props;
    }
  }]);
...
T.Datalist_ = TDatalist_;
T.Datalist = new TDatalist_();

var TOptgroup_ = function (_TSpan_29) {
  _inherits(TOptgroup_, _TSpan_29);
...
var TOption_ = function (_TSpan_30) {
  _inherits(TOption_, _TSpan_30);
...
var TB_ = function (_TSpan_31) {
  _inherits(TB_, _TSpan_31);

  function TB_(name, desc) {
    _classCallCheck(this, TB_);

    return _possibleConstructorReturn(this, (TB_.__proto__ || Object.getPrototypeOf(TB_)).call(this, name || 'B', desc));
  }

  _createClass2(TB_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TB_.prototype.__proto__ || Object.getPrototypeOf(TB_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'b';
      return props;
    }
  }]);
...
var TI_ = function (_TSpan_32) {
  _inherits(TI_, _TSpan_32);

  function TI_(name, desc) {
    _classCallCheck(this, TI_);

    return _possibleConstructorReturn(this, (TI_.__proto__ || Object.getPrototypeOf(TI_)).call(this, name || 'I', desc));
  }

  _createClass2(TI_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TI_.prototype.__proto__ || Object.getPrototypeOf(TI_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'i';
      return props;
    }
  }]);
...
var TS_ = function (_TSpan_33) {
  _inherits(TS_, _TSpan_33);

  function TS_(name, desc) {
    _classCallCheck(this, TS_);

    return _possibleConstructorReturn(this, (TS_.__proto__ || Object.getPrototypeOf(TS_)).call(this, name || 'S', desc));
  }

  _createClass2(TS_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TS_.prototype.__proto__ || Object.getPrototypeOf(TS_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 's';
      return props;
    }
  }]);
...
var TU_ = function (_TSpan_34) {
  _inherits(TU_, _TSpan_34);

  function TU_(name, desc) {
    _classCallCheck(this, TU_);

    return _possibleConstructorReturn(this, (TU_.__proto__ || Object.getPrototypeOf(TU_)).call(this, name || 'U', desc));
  }

  _createClass2(TU_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TU_.prototype.__proto__ || Object.getPrototypeOf(TU_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'u';
      return props;
    }
  }]);
...
var TIns_ = function (_TSpan_35) {
  _inherits(TIns_, _TSpan_35);

  function TIns_(name, desc) {
    _classCallCheck(this, TIns_);

    return _possibleConstructorReturn(this, (TIns_.__proto__ || Object.getPrototypeOf(TIns_)).call(this, name || 'Ins', desc));
  }

  _createClass2(TIns_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TIns_.prototype.__proto__ || Object.getPrototypeOf(TIns_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'ins';
      return props;
    }
  }]);
...
var TDel_ = function (_TSpan_36) {
  _inherits(TDel_, _TSpan_36);

  function TDel_(name, desc) {
    _classCallCheck(this, TDel_);

    return _possibleConstructorReturn(this, (TDel_.__proto__ || Object.getPrototypeOf(TDel_)).call(this, name || 'Del', desc));
  }

  _createClass2(TDel_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TDel_.prototype.__proto__ || Object.getPrototypeOf(TDel_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'del';
      return props;
    }
  }]);
...
var TCode_ = function (_TSpan_37) {
  _inherits(TCode_, _TSpan_37);

  function TCode_(name, desc) {
    _classCallCheck(this, TCode_);

    return _possibleConstructorReturn(this, (TCode_.__proto__ || Object.getPrototypeOf(TCode_)).call(this, name || 'Code', desc));
  }

  _createClass2(TCode_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TCode_.prototype.__proto__ || Object.getPrototypeOf(TCode_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'code';
      return props;
    }
  }]);
...
var TVar_ = function (_TSpan_38) {
  _inherits(TVar_, _TSpan_38);

  function TVar_(name, desc) {
    _classCallCheck(this, TVar_);

    return _possibleConstructorReturn(this, (TVar_.__proto__ || Object.getPrototypeOf(TVar_)).call(this, name || 'Var', desc));
  }

  _createClass2(TVar_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TVar_.prototype.__proto__ || Object.getPrototypeOf(TVar_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'var';
      return props;
    }
  }]);
...
var TSummary_ = function (_TSpan_39) {
  _inherits(TSummary_, _TSpan_39);

  function TSummary_(name, desc) {
    _classCallCheck(this, TSummary_);

    return _possibleConstructorReturn(this, (TSummary_.__proto__ || Object.getPrototypeOf(TSummary_)).call(this, name || 'Summary', desc));
  }

  _createClass2(TSummary_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TSummary_.prototype.__proto__ || Object.getPrototypeOf(TSummary_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'summary';
      return props;
    }
  }]);
...
T.Summary_ = TSummary_;
T.Summary = new TSummary_();

var TEm_ = function (_TSpan_40) {
  _inherits(TEm_, _TSpan_40);

  function TEm_(name, desc) {
    _classCallCheck(this, TEm_);

    return _possibleConstructorReturn(this, (TEm_.__proto__ || Object.getPrototypeOf(TEm_)).call(this, name || 'Em', desc));
  }

  _createClass2(TEm_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TEm_.prototype.__proto__ || Object.getPrototypeOf(TEm_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'em';
      return props;
    }
  }]);
...
var TStrong_ = function (_TSpan_41) {
  _inherits(TStrong_, _TSpan_41);

  function TStrong_(name, desc) {
    _classCallCheck(this, TStrong_);

    return _possibleConstructorReturn(this, (TStrong_.__proto__ || Object.getPrototypeOf(TStrong_)).call(this, name || 'Strong', desc));
  }

  _createClass2(TStrong_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TStrong_.prototype.__proto__ || Object.getPrototypeOf(TStrong_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'strong';
      return props;
    }
  }]);
...
T.Strong_ = TStrong_;
T.Strong = new TStrong_();

var TBig_ = function (_TSpan_42) {
  _inherits(TBig_, _TSpan_42);

  function TBig_(name, desc) {
    _classCallCheck(this, TBig_);

    return _possibleConstructorReturn(this, (TBig_.__proto__ || Object.getPrototypeOf(TBig_)).call(this, name || 'Big', desc));
  }

  _createClass2(TBig_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TBig_.prototype.__proto__ || Object.getPrototypeOf(TBig_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'big';
      return props;
    }
  }]);
...
var TSmall_ = function (_TSpan_43) {
  _inherits(TSmall_, _TSpan_43);

  function TSmall_(name, desc) {
    _classCallCheck(this, TSmall_);

    return _possibleConstructorReturn(this, (TSmall_.__proto__ || Object.getPrototypeOf(TSmall_)).call(this, name || 'Small', desc));
  }

  _createClass2(TSmall_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TSmall_.prototype.__proto__ || Object.getPrototypeOf(TSmall_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'small';
      return props;
    }
  }]);
...
var TDfn_ = function (_TSpan_44) {
  _inherits(TDfn_, _TSpan_44);

  function TDfn_(name, desc) {
    _classCallCheck(this, TDfn_);

    return _possibleConstructorReturn(this, (TDfn_.__proto__ || Object.getPrototypeOf(TDfn_)).call(this, name || 'Dfn', desc));
  }

  _createClass2(TDfn_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TDfn_.prototype.__proto__ || Object.getPrototypeOf(TDfn_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'dfn';
      return props;
    }
  }]);
...
var TSamp_ = function (_TSpan_45) {
  _inherits(TSamp_, _TSpan_45);

  function TSamp_(name, desc) {
    _classCallCheck(this, TSamp_);

    return _possibleConstructorReturn(this, (TSamp_.__proto__ || Object.getPrototypeOf(TSamp_)).call(this, name || 'Samp', desc));
  }

  _createClass2(TSamp_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TSamp_.prototype.__proto__ || Object.getPrototypeOf(TSamp_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'samp';
      return props;
    }
  }]);
...
var TKdb_ = function (_TSpan_46) {
  _inherits(TKdb_, _TSpan_46);

  function TKdb_(name, desc) {
    _classCallCheck(this, TKdb_);

    return _possibleConstructorReturn(this, (TKdb_.__proto__ || Object.getPrototypeOf(TKdb_)).call(this, name || 'Kdb', desc));
  }

  _createClass2(TKdb_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TKdb_.prototype.__proto__ || Object.getPrototypeOf(TKdb_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'kdb';
      return props;
    }
  }]);
...
var TCite_ = function (_TSpan_47) {
  _inherits(TCite_, _TSpan_47);

  function TCite_(name, desc) {
    _classCallCheck(this, TCite_);

    return _possibleConstructorReturn(this, (TCite_.__proto__ || Object.getPrototypeOf(TCite_)).call(this, name || 'Cite', desc));
  }

  _createClass2(TCite_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TCite_.prototype.__proto__ || Object.getPrototypeOf(TCite_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'cite';
      return props;
    }
  }]);
...
var TDialog_ = function (_TSpan_48) {
  _inherits(TDialog_, _TSpan_48);

  function TDialog_(name, desc) {
    _classCallCheck(this, TDialog_);

    return _possibleConstructorReturn(this, (TDialog_.__proto__ || Object.getPrototypeOf(TDialog_)).call(this, name || 'Dialog', desc));
  }

  _createClass2(TDialog_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TDialog_.prototype.__proto__ || Object.getPrototypeOf(TDialog_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'dialog';
      return props;
    }
  }]);
...
T.Dialog_ = TDialog_;
T.Dialog = new TDialog_();

var TMeter_ = function (_TSpan_49) {
  _inherits(TMeter_, _TSpan_49);
...
var TEmbed_ = function (_TSpan_50) {
  _inherits(TEmbed_, _TSpan_50);

  function TEmbed_(name, desc) {
    _classCallCheck(this, TEmbed_);

    return _possibleConstructorReturn(this, (TEmbed_.__proto__ || Object.getPrototypeOf(TEmbed_)).call(this, name || 'Embed', desc));
  }

  _createClass2(TEmbed_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TEmbed_.prototype.__proto__ || Object.getPrototypeOf(TEmbed_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'embed';
      return props;
    }
  }]);
...
var TObject_ = function (_TSpan_51) {
  _inherits(TObject_, _TSpan_51);

  function TObject_(name, desc) {
    _classCallCheck(this, TObject_);

    return _possibleConstructorReturn(this, (TObject_.__proto__ || Object.getPrototypeOf(TObject_)).call(this, name || 'Object', desc));
  }

  _createClass2(TObject_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TObject_.prototype.__proto__ || Object.getPrototypeOf(TObject_.prototype), 'getDefaultProps', this).call(this);
...
T.Object_ = TObject_;
T.Object = new TObject_();

var TParam_ = function (_TSpan_52) {
  _inherits(TParam_, _TSpan_52);

  function TParam_(name, desc) {
    _classCallCheck(this, TParam_);

    return _possibleConstructorReturn(this, (TParam_.__proto__ || Object.getPrototypeOf(TParam_)).call(this, name || 'Param', desc));
  }

  _createClass2(TParam_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TParam_.prototype.__proto__ || Object.getPrototypeOf(TParam_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'param';
      return props;
    }
  }]);
...
var TRuby_ = function (_TSpan_53) {
  _inherits(TRuby_, _TSpan_53);

  function TRuby_(name, desc) {
    _classCallCheck(this, TRuby_);

    return _possibleConstructorReturn(this, (TRuby_.__proto__ || Object.getPrototypeOf(TRuby_)).call(this, name || 'Ruby', desc));
  }

  _createClass2(TRuby_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TRuby_.prototype.__proto__ || Object.getPrototypeOf(TRuby_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'ruby';
      return props;
    }
  }]);
...
var TRp_ = function (_TSpan_54) {
  _inherits(TRp_, _TSpan_54);

  function TRp_(name, desc) {
    _classCallCheck(this, TRp_);

    return _possibleConstructorReturn(this, (TRp_.__proto__ || Object.getPrototypeOf(TRp_)).call(this, name || 'Rp', desc));
  }

  _createClass2(TRp_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TRp_.prototype.__proto__ || Object.getPrototypeOf(TRp_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'rp';
      return props;
    }
  }]);
...
var TRt_ = function (_TSpan_55) {
  _inherits(TRt_, _TSpan_55);

  function TRt_(name, desc) {
    _classCallCheck(this, TRt_);

    return _possibleConstructorReturn(this, (TRt_.__proto__ || Object.getPrototypeOf(TRt_)).call(this, name || 'Rt', desc));
  }

  _createClass2(TRt_, [{
    key: 'getDefaultProps',
    value: function getDefaultProps() {
      var props = _get(TRt_.prototype.__proto__ || Object.getPrototypeOf(TRt_.prototype), 'getDefaultProps', this).call(this);
      props['tagName.'] = 'rt';
      return props;
    }
  }]);
...
var TNavPanel_ = function (_TPanel_3) {
  ...
  function TNavPanel_(name, desc) {
    ...
    var _this112 = _possibleConstructorReturn(this, (TNavPanel_.__proto__ || Object.getPrototypeOf(TNavPanel_)).call(this, name || 'NavPanel', desc));

    _this112._slientProp.push('isNavigator.');
    return _this112;
  }
  ...
  _createClass2(TNavPanel_, [{
    ...
    key: 'setChecked',
    value: function setChecked(checkedId, callback) {
      if (checkedId && this.$gui.navItems[checkedId]) {
        // is valid checkedId
    ...
    key: 'fireChecked',
    value: function fireChecked(keyid, callback) {
      var widget = this.widget;
      if ((keyid || keyid === 0) && widget) {
        if (keyid == this.state.checkedId) {
          // checkedId not changed
          if (callback) callback();
          return;
        }
      ...
      function findOptNode(owner, keyid) {
        ...
        ownerObj.$gui.comps.forEach(function (child) {
          if (child) {
            var sKey = getElementKey_(child);if (sKey) b.push(sKey);
          }
        });
    ...
    value: function render() {
      ...
      var gui = this.$gui,
          bChild2 = gui.navHead.slice(0);
      var checkedId = this.state.checkedId;

      if (W.__design__) {
        // if in design, try show all (display:none for none-active playground)
        var bComp = gui.comps,
            iLen = bComp.length,
            bChild3 = [];
        for (var i = 0; i < iLen; i++) {
...
var TNavDiv_ = function (_TNavPanel_) {
  ...
  function TNavDiv_(name, desc) {
    ...
    var _this113 = _possibleConstructorReturn(this, (TNavDiv_.__proto__ || Object.getPrototypeOf(TNavDiv_)).call(this, name || 'NavDiv', desc));

    _this113._slientProp.push('isNavigator.');
    _this113._defaultProp.height = undefined;
    _this113._defaultProp.minHeight = 20;
    return _this113;
  }
...
function designRenderGround(obj, iGroundId) {
  ...
  if (!bChild.length && iGroundId == 2) bChild = obj.state['html.'] || null;
...
var TGroundPanel_ = function (_TPanel_4) {
  ...
  function TGroundPanel_(name, desc) {
    ...
    var _this114 = _possibleConstructorReturn(this, (TGroundPanel_.__proto__ || Object.getPrototypeOf(TGroundPanel_)).call(this, name || 'GroundPanel', desc));

    _this114._slientProp.push('isPlayground.');
    _this114._defaultProp.height = undefined;
    _this114._defaultProp.minHeight = 20;
    return _this114;
  }
...
var TGroundDiv_ = function (_TUnit_5) {
  ...
  function TGroundDiv_(name, desc) {
    ...
    var _this115 = _possibleConstructorReturn(this, (TGroundDiv_.__proto__ || Object.getPrototypeOf(TGroundDiv_)).call(this, name || 'GroundDiv', desc));

    _this115._slientProp.push('tagName.');
    _this115._slientProp.push('isPlayground.');
    _this115._defaultProp.height = undefined;
    _this115._defaultProp.minHeight = 20;
    return _this115;
  }
  ...
  function setupSpace(space) {
    ...
    function makePropsDualsFn(sName) {
      return function (idxOrPath) {
        ...
        if (tp == 'number') {
          if (path_ < 0) {
            // only support 0 layer callspace
        ...
        if (path_ === 0) return thisObj[sName]; // quick found
        else {
...
function fireOptTrigger_(thisObj) {
  ...
  var exprSpace = triggerInfo[0],
      bAst = triggerInfo[1];
  exprSpace.$info = [thisObj, 'trigger', ex.time()];
  var bRet = evalInSpace(bAst, exprSpace);
  // delete exprSpace.$info;   // keep for asynchrony task

  var tp = typeof bRet === 'undefined' ? 'undefined' : _typeof(bRet);
  if (tp == 'string') bRet = [bRet];else if (tp == 'object') {
    if (!Array.isArray(bRet)) bRet = [['', bRet]];
  } else return; // unknown format

  var item = bRet.shift();
  while (true) {
    if (Array.isArray(item)) triggerData(item[0] || '', item[1]); // update duals.data
    else if (typeof item == 'string') {
        var next,
            cfgData = null;
        if (bRet.length > 0 && _typeof(next = bRet[0]) == 'object' && !Array.isArray(next)) cfgData = bRet.shift();
  ...
  function triggerCheck(sPath, cfgData) {
    ...
    setTimeout(function () {
      ...
      if (targ.props['isOption.'] && targ.setChecked) targ.setChecked(null, false, dCfg); // callback is null, isForce is false
    }, 0);
  }
}
  ...
  while (owner) {
    ...
    if (ownerObj) {
      if (ownerObj.props['isNavigator.']) {
        if (ownerObj.setChecked) findNav = true;
        break;
...
var TOptSpan_ = function (_TSpan_56) {
  _inherits(TOptSpan_, _TSpan_56);

  function TOptSpan_(name, desc) {
    _classCallCheck(this, TOptSpan_);

    var _this116 = _possibleConstructorReturn(this, (TOptSpan_.__proto__ || Object.getPrototypeOf(TOptSpan_)).call(this, name || 'OptSpan', desc));

    _this116._slientProp.push('isOption.');
    return _this116;
  }
  ...
  _createClass2(TOptSpan_, [{
    ...
    key: 'componentDidMount',
    value: function componentDidMount() {
      _get(TOptSpan_.prototype.__proto__ || Object.getPrototypeOf(TOptSpan_.prototype), 'componentDidMount', this).call(this);
      ...
      if (this.state['data-checked']) {
        ...
        setTimeout(function () {
          ...
          if (needSync) {
            // change selected not cause parent <select>'s onchange event
            var owner = self.widget;
            owner = owner && owner.parent;
            var ownerObj = owner && owner.component;
            ...
            if (ownerObj && ownerObj.syncSelected) ownerObj.syncSelected();
          }
    ...
    key: 'setChecked',
    value: function setChecked(callback, isForce, newOpt) {
      // isForce means force re-selelect
      var justSet = false,
          recheckable = this.state.recheckable;
      var ownerObj = null,
          self = this;

      var popOption = this.state.popOption;
      if (popOption && newOpt) popOption = this.state.popOption = Object.assign({}, popOption, newOpt);
      ...
      function trySyncChecked() {
        ...
        if (self.props['tagName.'] == 'option') {
          ...
          if (thisNode && (!thisNode.selected || isForce || recheckable)) {
            if (!thisNode.selected) thisNode.selected = true;
          ...
          var thisNode = ReactDOM.findDOMNode(self);
          var sTag = self.props['tagName.'] || 'div',
              nodes = [];
          try {
            nodes = ownerNode.querySelectorAll(sTag + '[name="' + sName + '"]');
          } catch (e) {}
          for (var i = 0, node; node = nodes[i]; i++) {
            if (node !== thisNode && node._syncChecked) node._syncChecked(false); // set all others: data-checked = ''
          }
        }
        // else ignore
      }
      ...
      if (ownerObj) {
        ...
        if (keyid != ownerObj.getChecked() || isForce || recheckable) {
          // if recheckable, can recall ownerObj.setChecked()
          if (!ownerObj.canNavigateTo || ownerObj.canNavigateTo(keyid)) {
            if (isForce || recheckable) justSet = true;
            if (!this.state['data-checked']) {
              // at least set checked once
              justSet = true;
              this.duals['data-checked'] = '1';
            }
...
var TOptImg_ = function (_TOptSpan_2) {
  ...
  function TOptImg_(name, desc) {
    ...
    return _possibleConstructorReturn(this, (TOptImg_.__proto__ || Object.getPrototypeOf(TOptImg_)).call(this, name || 'OptImg', desc));
  }
...
var TOptInput_ = function (_TOptSpan_4) {
  _inherits(TOptInput_, _TOptSpan_4);

  function TOptInput_(name, desc) {
    _classCallCheck(this, TOptInput_);

    return _possibleConstructorReturn(this, (TOptInput_.__proto__ || Object.getPrototypeOf(TOptInput_)).call(this, name || 'OptInput', desc));
  }
  ...
  _createClass2(TOptInput_, [{
    ...
    key: 'componentDidMount',
    value: function componentDidMount() {
      _get(TOptInput_.prototype.__proto__ || Object.getPrototypeOf(TOptInput_.prototype), 'componentDidMount', this).call(this);
      ...
      if (this.state['data-checked']) {
        setTimeout(function () {
          var navObj = findNavOwner_(self);
          if (navObj && !navObj.$gui.inFiring) self.setChecked(null); // force=false
        }, 0); // wait next process to ensure widget loaded
      }
    }
  }, {
    key: 'doChange',
    value: function doChange(toChecked, callback, isForce, newOpt) {
      // called by radio or checkbox
      var self = this,
          newChecked = toChecked;
      ...
      if (self.state.isolated) {
        ...
        if (toChecked && (changed || isForce || recheckable)) canFire = true;
        return doCallback();
      }
      ...
      if (ownerObj) {
        ...
        if (toChecked && (ownerObj.getChecked() != keyid || isForce || recheckable)) {
          if (ownerObj.canNavigateTo && !ownerObj.canNavigateTo(keyid)) newChecked = false; // can not change checked state
          else {
              self.duals['data-checked'] = '1';
    ...
    value: function setChecked(callback, isForce, newOpt) {
      ...
      self.doChange(true, function (newChecked) {
        if (targ.checked != newChecked) targ.checked = newChecked;

        var sName = self.state.name;
        if (sName && self.props.type == 'radio') {
          // synchronize radio group
          var ownerObj = self.findNavOwner(),
              ownerNode = ownerObj && ReactDOM.findDOMNode(ownerObj);
          if (!ownerNode) return;

          var nodes = ownerNode.querySelectorAll('input[type="radio"]');
          for (var i = 0, node; node = nodes[i]; i++) {
            if (node._syncChecked && node.getAttribute('name') === sName) node._syncChecked();
          }
        }
      ...
      function performChange(self, targ) {
        ...
        if (checkableInput_.indexOf(sType) >= 0) {
          self.doChange(targ.checked, function (newChecked) {
            if (targ.checked != newChecked) targ.checked = newChecked; // ignore further $onChange() // maybe fire $$onChange again

            var sName = self.state.name;
            if (sName && sType == 'radio') {
              // synchronize radio group
              var ownerObj = self.findNavOwner(),
                  ownerNode = ownerObj && ReactDOM.findDOMNode(ownerObj);
              if (!ownerNode) return;
          ...
          // none-checkable, just keep content synchronized, not fire setChecked()
...
var TOptOption_ = function (_TOptSpan_5) {
  _inherits(TOptOption_, _TOptSpan_5);

  function TOptOption_(name, desc) {
    _classCallCheck(this, TOptOption_);

    return _possibleConstructorReturn(this, (TOptOption_.__proto__ || Object.getPrototypeOf(TOptOption_)).call(this, name || 'OptOption', desc));
  }
...
function setOptChecked_(self, callback, isForce, newOpt) {
  ...
  var popOption = self.state.popOption;
  if (popOption && newOpt) popOption = self.state.popOption = Object.assign({}, popOption, newOpt);
  if (popOption && typeof popOption.path == 'string') {
    if (!popOption.opened && !self.state.isolated) {
      var sPath = popOption.path;
      if (!sPath) sPath = './' + self.$gui.keyid;
...
var TOptDiv_ = function (_TDiv_8) {
  _inherits(TOptDiv_, _TDiv_8);

  function TOptDiv_(name, desc) {
    _classCallCheck(this, TOptDiv_);

    var _this122 = _possibleConstructorReturn(this, (TOptDiv_.__proto__ || Object.getPrototypeOf(TOptDiv_)).call(this, name || 'OptDiv', desc));

    _this122._slientProp.push('isOption.');
    return _this122;
  }
  ...
  _createClass2(TOptDiv_, [{
    ...
    key: 'componentDidMount',
    value: function componentDidMount() {
      _get(TOptDiv_.prototype.__proto__ || Object.getPrototypeOf(TOptDiv_.prototype), 'componentDidMount', this).call(this);
...
var TOptLi_ = function (_TP_39) {
  _inherits(TOptLi_, _TP_39);

  function TOptLi_(name, desc) {
    _classCallCheck(this, TOptLi_);

    var _this123 = _possibleConstructorReturn(this, (TOptLi_.__proto__ || Object.getPrototypeOf(TOptLi_)).call(this, name || 'OptLi', desc));

    _this123._slientProp.push('isOption.');
    return _this123;
  }
  ...
  _createClass2(TOptLi_, [{
    ...
    key: 'componentDidMount',
    value: function componentDidMount() {
      _get(TOptLi_.prototype.__proto__ || Object.getPrototypeOf(TOptLi_.prototype), 'componentDidMount', this).call(this);
      checkDefChecked_(this);
    }
  }, {
    key: 'findNavOwner',
...
function setTemplateChild_(temp, child) {
  ...
  if (child.props['isReference.']) {
    ...
    ele = React.cloneElement(child, { 'hookTo.': temp, 'keyid.': keyid_, key: sPath });
  } else {
    ele = React.cloneElement(child, { 'hookTo.': temp, 'keyid.': keyid_, key: sPath });
  ...
  function scanEveryChild(temp, sPath, bInfo, ownerSeg) {
    ...
    React.Children.forEach(children, function (child, idx) {
      ...
      var sKeyid = keyid + '';
      var iFlag2 = 1,
          ele2 = React.cloneElement(child, { 'keyid.': keyid, key: sKeyid, 'hookTo.': temp });
...
var TTempPanel_ = function (_TPanel_5) {
  ...
  function TTempPanel_(name, desc) {
    ...
    var _this124 = _possibleConstructorReturn(this, (TTempPanel_.__proto__ || Object.getPrototypeOf(TTempPanel_)).call(this, name || 'TempPanel', desc));

    _this124._slientProp.push('isTemplate.', 'data-temp.type');
    return _this124;
  }
...
var TTempDiv_ = function (_TUnit_6) {
  ...
  function TTempDiv_(name, desc) {
    ...
    var _this125 = _possibleConstructorReturn(this, (TTempDiv_.__proto__ || Object.getPrototypeOf(TTempDiv_)).call(this, name || 'TempDiv', desc));

    _this125._slientProp.push('isTemplate.', 'data-temp.type');
    return _this125;
  }
...
var TTempSpan_ = function (_TSpan_57) {
  _inherits(TTempSpan_, _TSpan_57);

  function TTempSpan_(name, desc) {
    _classCallCheck(this, TTempSpan_);

    var _this126 = _possibleConstructorReturn(this, (TTempSpan_.__proto__ || Object.getPrototypeOf(TTempSpan_)).call(this, name || 'TempSpan', desc));

    _this126._slientProp.push('isTemplate.', 'data-temp.type');
    return _this126;
  }
...
var TRefDiv_ = function (_TUnit_7) {
  ...
  function TRefDiv_(name, desc) {
    ...
    var _this127 = _possibleConstructorReturn(this, (TRefDiv_.__proto__ || Object.getPrototypeOf(TRefDiv_)).call(this, name || 'RefDiv', desc));

    _this127._slientProp.push('isReference.');
    return _this127;
  }
  ...
  _createClass2(TRefDiv_, [{
    ...
    value: function getInitialState() {
      ...
      if (this.props['hookTo.'] === topmostWidget_) utils.instantShow('warning: can not add RefDiv to topmost widget.');
      return dState;
    }
  }, {
    key: 'componentDidMount',
    ...
    value: function render() {
      ...
      return React.createElement('div', dProp);
    }
  }]);
...
var TRefSpan_ = function (_TSpan_58) {
  _inherits(TRefSpan_, _TSpan_58);

  function TRefSpan_(name, desc) {
    _classCallCheck(this, TRefSpan_);

    var _this128 = _possibleConstructorReturn(this, (TRefSpan_.__proto__ || Object.getPrototypeOf(TRefSpan_)).call(this, name || 'RefSpan', desc));

    _this128._slientProp.push('isReference.');
    return _this128;
  }
  ...
  _createClass2(TRefSpan_, [{
    ...
    value: function getInitialState() {
      ...
      if (this.props['hookTo.'] === topmostWidget_) utils.instantShow('error: can not hook RefSpan to topmost widget.');
      return dState;
    }
  }, {
    key: 'componentDidMount',
    ...
    value: function render() {
      ...
      var dProp = { style: { width: '0px', height: '0px' } };
      if (sCls) dProp.className = sCls;
      return React.createElement('span', dProp);
    }
  }]);
...
var TScenePage_ = function (_TPanel_6) {
  ...
  function TScenePage_(name, desc) {
    ...
    var _this129 = _possibleConstructorReturn(this, (TScenePage_.__proto__ || Object.getPrototypeOf(TScenePage_)).call(this, name || 'ScenePage', desc));

    _this129._slientProp.push('isScenePage.');
    return _this129;
  }
  ...
  _createClass2(TScenePage_, [{
    ...
    value: function getInitialState() {
      ...
      if (this.props['hookTo.'] !== topmostWidget_) utils.instantShow('error: ScenePage only hook to topmost widget.');

      var initStyle = { position: 'absolute', display: 'none' }; // fixed to absolute, default hidden
      if (!this.props.zIndex) initStyle.zIndex = '0'; // default is 0
      ...
      if (W.__design__) this.$gui.currSelected = '';
      return dState;
    }
  }, {
    key: 'setSelected',
    value: function setSelected(sKey) {
      ...
      function setSelected_(sKey, isSelect) {
        ...
        if (isSelect) {
          ...
          // restore to normal
          if (iLevel >= 1001 && iLevel <= 2999) childObj.setState({ style: Object.assign({}, childObj.state.style, { zIndex: iLevel - 2000 }) });
        }
      }
    }
  }, {
  ...
  _createClass2(TMaskPanel_, [{
      ...
      this.defineDual('id__', function (value, oldValue) {
        ...
        if (popOption) {
          ...
          if (!dFrame) dFrame = {};else {
            ...
            if (frameEle && frame.props) noFrame = false;else frameEle = null;
          }

          if (this.props.children) {
            var b = React.Children.toArray(this.props.children); // not use childElements_()
        ...
        if (bodyEle && hookOwner && containNode_) {
          ...
          if (frameEle) {
            var iWd = parsePercent(dFrame.width || 0.9, stWd, 60);
            var iHi = parsePercent(dFrame.height || 0.9, stHi, 60);
            ...
            gui.compIdx['0'] = 0;
            gui.comps.push(React.cloneElement(frameEle, dProp_));
          }

          // step 3: try set default frame style if popOption.frame not passed
          var iWd2 = parsePercent(popOption.width || 0.8, stWd, 40);
          var iHi2 = parsePercent(popOption.height || 0.8, stHi, 40);
          var iLeft2 = parseLeftTop(popOption.left, stWd, iWd2);
          var iTop2 = parseLeftTop(popOption.top, stHi, iHi2);

          if (noFrame && iLeft2 > 0 && iTop2 > 0 && iLeft2 + iWd2 < stWd && iTop2 + iHi2 < stHi) {
            var iLeft3 = iLeft2 - 1,
                iTop3 = iTop2 - 1;
            frameEle = React.createElement('div', { key: '0', style: {
    ...
    value: function componentDidMount() {
      ...
      if (node) {
        ...
        var winIdx = this.$gui.winComp;
        var ele = typeof winIdx == 'number' ? this.$gui.comps[winIdx] : null;
        if (W.__design__ || !ele) node.onclick = noPropagation;else {
          var sKey = getElementKey_(ele);
          var winObj = this.widget && sKey && this.widget[sKey]; // get window widget in children
          winObj = winObj && winObj.component;
          var winNode = winObj && ReactDOM.findDOMNode(winObj);
          if (winNode) winNode.onclick = noPropagation; // avoid click window area leading to close

          var byManual = this.state.popOption;
          byManual = byManual && byManual.manualClose;
...
var TMarkedDiv_ = function (_TDiv_9) {
  _inherits(TMarkedDiv_, _TDiv_9);

  function TMarkedDiv_(name, desc) {
    _classCallCheck(this, TMarkedDiv_);

    var _this131 = _possibleConstructorReturn(this, (TMarkedDiv_.__proto__ || Object.getPrototypeOf(TMarkedDiv_)).call(this, name || 'MarkedDiv', desc));

    _this131._statedProp.push('mark');
    _this131._slientProp.push('hasStatic.', 'noSaveChild.');
    return _this131;
  }
  ...
  _createClass2(TMarkedDiv_, [{
          ...
          if (typeof sText == 'string') {
            renewMarkdown_(compObj, sText, function (succ) {
              ...
              compObj.setState({ mark: escape(sText) }, function () {
                ...
                if (W.__design__) {
                  // notify backup current doc
                  if (containNode_ && containNode_.notifyBackup) containNode_.notifyBackup(sWdgtPath); // renew schema prop editor
                }
    ...
    key: 'getInitialState',
    value: function getInitialState() {
      var dState = _get(TMarkedDiv_.prototype.__proto__ || Object.getPrototypeOf(TMarkedDiv_.prototype), 'getInitialState', this).call(this);
...
},{"./react_widget":2,"react":undefined,"react-dom":undefined}],4:[function(require,module,exports){
