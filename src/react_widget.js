...
W.$creator.textableBlock = function(sTemplate) {  // for block tag, all inline tag is textable
    ...
    function scanJsxNode(container) {
      ...
      function scanNodeAttr(node,sPrefix,idx) {
        ...
        for (var i2=0; i2 < attrLen; i2++) {
          var item = node.attributes[i2], sName = item.name;
          if (sName == ':t' || sName == '$') {
        ...
        if (sTemplate) {
          ...
          bProp.forEach( function(item,idx) {
            var sAttr = item[0].toLowerCase(), sValue = item[1];
            var b = sAttr.split('-');
          ...
          if (node.children.length == 0 && (isSpan || _textableTag[sTemplate])) {  // <span :t=Span>htmlText</span>
